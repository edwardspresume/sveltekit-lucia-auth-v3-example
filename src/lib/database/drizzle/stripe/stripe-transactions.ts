import { validateSubscriptionStatus, validateTransactionStatus } from './validations';
import { sqliteDb as db } from '../db';
import { subscriptions, transactions } from './turso-stripe-schema';



export async function createSubscription(userId: number, productId: number, status: string) {
...
export async function createTransaction(subscriptionId: number, amountCents: number, status: string) {
